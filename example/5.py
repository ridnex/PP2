...
sql = f"select * from snakebook where user_login =\'{login_name}\'"
cursor.execute(sql)
...
if len(info) > 0:
    ...
    for i in range(snake_len):
        sql_snake_x=f"SELECT snake_array[{i}+1][1] from snakebook where user_login =\'{login_name}\'"
        sql_snake_y=f"SELECT snake_array[{i}+1][2] from snakebook where user_login =\'{login_name}\'"
        cursor.execute(sql_snake_x)
        snake_x = cursor.fetchall()
        body[i][0]= snake_x
        print(body[0][0])
        cursor.execute(sql_snake_y)
        snake_y = cursor.fetchall()
        body[i][1]= snake_y
    for i in range(wall_len):
        sql_wall_x=f"SELECT wall_array[{i}+1][1] from snakebook where user_login =\'{login_name}\'"
        sql_wall_y=f"SELECT wall_array[{i}+1][2] from snakebook where user_login =\'{login_name}\'"
        cursor.execute(sql_wall_x)
        wall_x = cursor.fetchall()
        wall[i][0]= wall_x
        cursor.execute(sql_wall_y)
        wall_y = cursor.fetchall()
        body[i][1]= wall_y
    record = info[0][8]
    ...
    sql_insert = f"INSERT INTO snakebook(user_login, last_score, last_level, last_FPS, snake_len, wall_len, snake_array, wall_array, record) VALUES( \'{login_name}\',\'{score_value}\',\'{level_value}\',\'{FPS}\', \'{snake_len}\', \'{wall_len}\','{{200,45}}', '{{20,40}}', \'{record}\' )"
    cursor.execute(sql_insert)
    cursor.executemany('UPDATE snakebook SET snake_array = %s where user_login =\'{login_name}\'',(body, ))    
    #cursor.executemany('UPDATE snakebook SET wall_array = %s where user_login =\'{login_name}\'',(wall, ))

print("head element: ", body[0][0])
print("head arr: ", body[0])
...
def GAME_OVER():
    ...
    sql_insert1 = f"UPDATE snakebook set  last_score = 0, last_level = 1, last_FPS = 10, snake_len = 1, wall_len = 0  where user_login = \'{login_name}\' "
    cursor.execute(sql_insert1)
    if score_value > record:
        sql_insert1 = f"UPDATE snakebook set record = \'{score_value}\'  where user_login = \'{login_name}\' "
        record = score_value
        cursor.execute(sql_insert1)
        cursor.executemany('UPDATE snakebook SET snake_array = %s where user_login =\'{login_name}\'',([[10,10]],))    
        cursor.executemany('UPDATE snakebook SET wall_array = %s where user_login =\'{login_name}\'',([[-10,-10]],))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_p:
                print("POUSE")
                sql_insert = f"UPDATE snakebook set last_score =\'{score_value}\',last_level =\'{level_value}\',last_FPS =\'{FPS}\',snake_len=\'{snake_len}\', record = \'{record}\' where user_login = \'{login_name}\'; "
                cursor.execute(sql_insert)
                print("boady arr: ", body)
                cursor.executemany('UPDATE snakebook SET snake_array = %s where user_login =\'{login_name}\'',(body,))    
                cursor.executemany('UPDATE snakebook SET wall_array = %s where user_login =\'{login_name}\'',(wall,))
    ...
    text4 = font.render("Record: "+str(record), True, WHITE)
